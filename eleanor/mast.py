...
def coords_from_tic(tic):
    ...
    return [ticData['ra'].data[0], ticData['dec'].data[0]], ticData['Tmag'].data, ticData['version']
...
def tic_from_coords(coords):
    ...
    return int(tess[sepTess==np.min(sepTess)]['MatchID'].data[0]), [tess[sepTess==np.min(sepTess)]['Tmag'].data[0]], sepTess[sepTess==np.min(sepTess)]/u.arcsec, tess['version'][0]
