...
class Visualize(object):
    ...
    def _add_gaia_figure_elements(self, tpf, fig, magnitude_limit=18):
        ...
        result = gaia_sources_in_tpf(tpf, magnitude_limit=magnitude_limit)
        ...
        coords_x, coords_y = result["coords_x"], result["coords_y"]

        # Gently size the points by their Gaia magnitude
        sizes = 10000.0 / 2**(result['Gmag']/2)

        plt.scatter(coords_x+tpf.shape[1]/2+tpf.column, coords_y+tpf.shape[2]/2+tpf.row, c='firebrick', alpha=0.5, edgecolors='r', s=sizes)
        plt.scatter(coords_x+tpf.shape[1]/2+tpf.column, coords_y+tpf.shape[2]/2+tpf.row, c='None', edgecolors='r', s=sizes)
