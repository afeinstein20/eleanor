...
* TargetData objects
        ...
        In the 0th extension, the pixel-level data is stored. Users might find this extension similar to a Kepler ``TPF'' file. In the 1st extension is the light curve for eleanor's estimate of the optimal aperture for the target, both with and without accounting for instrumental systematics. Users might find this extension similar to a Kepler ``LLC'' file.
        The results found with other apertures are stored in additional extensions.
        If a target is observed for multiple sectors, eleanor will produce target pixel file and light curve data for the star for every sector that it is
        ``on silicon.'' Alternatively, the user can request data for only a specific sector.
